...
class Import_Graph:
    ...
    @staticmethod
    def make_graph(node_file, edges_file, layers_file=None, inter_layers_file=None):
        nodes = []
        pos = {}
        file = open(node_file)
        for line in file:
            node = line.split(" ")
            nodes.append((int(node[0]), {"name": node[1]}))
        file.close()
        ...
        if layers_file:
            ...
            for line in file:
                layer = line.split(" ")
                layers.append((int(layer[0]), layer[1]))
...
# If layers is not provided (though the condition len(layers) < 0 will always be false), creates a unique set of layer IDs from the edges and converts it to a list
        ...
        mg = mp.MultiNetPy(list_of_layers=graphs, inter_adjacency_matrix=inter_layers_file)
        return mg
